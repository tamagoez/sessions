...
export function getStatus(id) {
  ...
  function getfunc() {
    ...
    if (lastobject["status"][id] === undefined) {
      ...
      console.log("[getfunc] got: " + lastobject["status"][id]);
...
export function getDate(id) {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      dealDate(id)
      .then((value) => {setReval(value)})
      .then(() => localStorage.setItem("ignore_scroll", "true"))
    }, 9000);
    return () => {
      clearInterval(intervalId);
    };
