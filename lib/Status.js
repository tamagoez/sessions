...
export function getStatus(id) {
  ...
  function getfunc() {
    if (!localStorage.getItem("user_status")) {
      ...
      const newObject = {
        expiresAt: nowdate.getTime() + settingobject["status_load_interval"]
      }
      localStorage.setItem("user_status", newObject)
    } else {
    const lsobject = localStorage.getItem("user_status")
    ...
    if (lsobject["expiresAt"] > nowdate.getTime()) {
      ...
      const newObject = {
        expiresAt: objectnowdate.getTime() + settingobject["status_load_interval"]
      }
      localStorage.setItem("user_status", newObject)
    } else {
    if (!lsobject["status"][id]) {
      const returnbool = loaddeal(id);
      const dataObject = lsobject;
      returnbool.then((value) => {
        console.log("[getStatus] returnbool: " + value);
        dataObject["status"][id] = value;
        localStorage.setItem("user_status", dataObject);
        setReturnval(value);
      });
    } else {
      setReturnval(lsobject["status"][id])
    }
    }
    }
  }
