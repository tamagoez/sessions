...
export const addMessage = async (message, channel, userid) => {
  // const encrypted = ncrypt.encrypt(message, getkey(userid, channel));
  try {
    let { body } = await supabase.from('channels_chat').insert([{ message: message, channel, userid }])
    return body
  } catch (error) {
    alert('error' + error)
  }
